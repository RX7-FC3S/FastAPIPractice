...
@router.post("/add_bin_info", response_model=schema.Response.AddBinInfo, tags=["库位信息", "增"])
def add_bin_info(params: schema.Request.AddBinInfo, db: Session = Depends(create_session)):
    try:
        db_bin_info = crud.crud_bin_info.get_by_bin_code(db, params.bin_code)

        if db_bin_info:
            return Response(False, f"BinInfo(bin_code:{params.bin_code}) already exists", None)
        db_bin_info = crud.crud_bin_info.get_bin_info_by_row_col_and_level(db, params.row, params.col, params.level)

        if db_bin_info:
            return Response(
                False, f"BinInfo(row:{params.row}, col:{params.col}, level:{params.level}) already exists", None
            )
        db_bin_spec = crud_bin_spec.get(db, params.bin_spec_id)
        if not db_bin_spec:
            return Response(False, f"BinSpec(id:{params.bin_spec_id}) does not exists", None)
        db_warehouse_area = crud_warehouse_area.get(db, params.warehouse_area_id)
        if not db_warehouse_area:
            return Response(False, f"WarehouseArea(id:{params.warehouse_area_id}) does not exists", None)
...
@router.delete("/delete_bin_info", response_model=schema.Response.DeleteBinInfo, tags=["库位信息", "删"])
def delete_bin_info(params: schema.Request.DeleteBinInfo, db: Session = Depends(create_session)):
    try:
