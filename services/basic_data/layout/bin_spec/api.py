...
@router.post(
    "/add_bin_spec", response_model=schema.Response.AddBinSpec, tags=["库位规格"]
)
async def add_bin_spec(
    params: schema.Request.AddBinSpec, db: Session = Depends(create_session)
):
    try:
        ...
        return Response(
            True, "", crud.crud_bin_spec.add(db, model.BinSpec(**params.model_dump()))
        )
...
@router.post(
    "/get_bin_specs", response_model=schema.Response.GetBinSpecs, tags=["库位规格"]
)
async def get_bin_specs(
    ...
    try:
        return Response(
            ...
            "",
            paginate(
                crud.crud_bin_spec.get_bin_specs(db, query_params, order_params),
                pagination_params,
            ),
        )
    except Exception as e:
...
@router.get(
    "/get_bin_specs_id_and_name",
    response_model=schema.Response.GetBinSpecsIdAndName,
    tags=["库位规格"],
)
